...
namespace FireArmsInventoryManagementSystem.Models
{
    public abstract class Firearm : IValidatableObject
    {
        ...
        [Required, StringLength(100)] public string Manufacturer { get; set; } = default!;
        [StringLength(100)] public string? Importer { get; set; }
        [StringLength(50)] public string? ImporterCity { get; set; }
        public USState? ImporterState { get; set; }

        [Required, StringLength(50)] public string ManufacturerCity { get; set; } = default!;
        [Required] public USState ManufacturerState { get; set; }

        // Model & serial
        [Required, StringLength(100)] public string Model { get; set; } = default!;
        ...
        [StringLength(100)] public string? AdditionalMarkings { get; set; }
        [Required] public bool IsSerialObliterated { get; set; }

        // Caliber
        [Required] public int CaliberId { get; set; }
        [ForeignKey(nameof(CaliberId))] public Caliber Caliber { get; set; } = default!;
        ...
        [StringLength(200)] public string? OtherTypeDescription { get; set; }
        [NotMapped] public bool IsFrameOrReceiver => Type == FirearmType.Receiver;

        // Manufacture details
        [Required, DataType(DataType.Date)] public DateTime ManufactureDate { get; set; }
        [Required, StringLength(100)] public string CountryOfOrigin { get; set; } = default!;
        public bool IsAntique { get; set; }
        [Required] public bool IsImported { get; set; }

        // FFL tracking
        [Required] public int FFLId { get; set; }
        [ForeignKey(nameof(FFLId))] public FFL FFL { get; set; } = default!;
        [StringLength(50)] public string? YourFFLMarking { get; set; }
        [StringLength(50)] public string? YourMarkingLocation { get; set; }
        ...
        [JsonIgnore] public string? InternalNotes { get; set; }
        ...
        public virtual IEnumerable<ValidationResult> Validate(ValidationContext _)
        {
            ...
            if (IsImported)
            {
                if (string.IsNullOrWhiteSpace(Importer) || string.IsNullOrWhiteSpace(ImporterCity) || !ImporterState.HasValue)
